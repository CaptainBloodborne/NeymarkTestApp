...
namespace ORU
{
    ...
    void ServerService::destroyCommunicationServices()
    {
        ...
        close(_serverFd);
        syslog(LOG_INFO, "Removing the Unix domain socket file...");
        unlink(_socketPath.c_str());
        syslog(LOG_INFO, "Removing the pid file...");
    ...
    void ServerService::processMessage()
    {
        ...
        syslog(LOG_INFO, "Received %d bytes", numBytes);
        ...
        if (numBytes > 0)
        {
            ...
            syslog(LOG_INFO,
                   ...
                   static_cast<int>(header->seqNum),
                   static_cast<int>(header->type),
                   static_cast<int>(header->size),
                   static_cast<int>(header->status));
            ...
            case MSG_PING:
            {
                ...
                syslog(LOG_INFO, "Receive PING message");
            ...
            case MSG_REQ_STATUS:
            {
                ...
                syslog(LOG_INFO, "Receive REQ_STATUS message");
            ...
            default:
            {
                ...
                syslog(LOG_WARNING, "Receive REQ_STATUS message");
    ...
    void ServerService::setPidFile()
    {
        ...
        else
        {
            ...
            syslog(LOG_ERR, "Failed to set PID to file");
        }
    }
    ...
    int ServerService::run()
    {
        ...
        syslog(LOG_INFO, "Server was started");

        int res = createCommunicationServices();
        if (res)
        {
            // cout << "Failed to create communication service" << endl;
            syslog(LOG_ERR, "Failed to create communication service");
            return -1;
        }

        // cout << "Communication service was successfully created" << endl;
        syslog(LOG_INFO, "Communication service was successfully created");

        int pid = fork();
        if (pid == -1) // error
        {
            // cout << "Failed to run service" << endl;
            syslog(LOG_ERR, "Failed to run service");
            return -1;
        }
    ...
    int ServerService::quit()
    {
        ...
        syslog(LOG_INFO, "Server was stoped");
        closelog();
        return 0;
    }
